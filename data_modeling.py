...
def main():
    ...
    query = """
        ...
        FROM
         top_tracks t1
        JOIN
         audio_features t2 ON t2.id = t1.id AND CAST(t1.dt AS DATE) = DATE('2020-01-20') AND CAST(t2.dt AS DATE) = DATE('2020-01-20')
    ...
    metrics = ["danceability", "energy", "loudness", "acousticness", "instrumentalness"]
...
def query_athena(query, athena):
    response = athena.start_query_execution(
        QueryString=query,
        QueryExecutionContext={"Database": "fastcampus"},
        ResultConfiguration={
            "OutputLocation": "s3://feud72-query-results-bucket/",
